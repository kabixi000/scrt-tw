...
print('Home timeline (30 tweets):')
print('-' * terminal_size)
home_timeline = api.home_timeline(count=30)
for status in home_timeline:
    print(status.created_at.astimezone(tz=ZoneInfo('Asia/Tokyo')))
    print(status.user.name)
    print(status.text)
    # print(status.truncated)
    # if status.truncated:
    #     ex_status = api.get_status(status.id, tweet_mode="extended")
    #     print(ex_status.text)
    print('-' * terminal_size)
print('=' * terminal_size)
